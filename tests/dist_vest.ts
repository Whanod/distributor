...
import { generateCsvFromEntries,generateCsvFromVestedEntries } from "./utils/generate_csv";
...
import { TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, createMint, getOrCreateAssociatedTokenAccount, getAssociatedTokenAddress, mintTo, createAccount } from "@solana/spl-token";
...
describe("Create A Distributor Instance and claim", () => {
  it("should create a distributor", async () => {
    ...
    await generateCsvFromVestedEntries("distributor_vested.csv", users.map(user => ({
      address: user.publicKey.toBase58(),
      amount_unlocked: 1000,
      amount_locked: 2000
    })));
...
if (fs.existsSync(distributorPath)) {
  ...
  console.log("Distributor folder deleted");
    ...
    generateMerkleProofs("distributor_vested.csv", "distributor");
...
console.log("max num nodes:",max_num_nodes)
  ...
  it("should create a claim instance", async () => {
    ...
    for (let user of users) {
      ...
      let ix = await program.methods.newClaim(new BN (proof.amount_unlocked),new BN(proof.amount_locked),proof.proof).accounts({
        distributor: distributorPdaAndBump[0],
     ...
     await provider.connection.confirmTransaction(airdrop_signature,"confirmed")
      ...
      provider.connection.confirmTransaction(x)}
      catch(e){
        console.log("error:",e)
      }
...
await wait(2000)
  ...
  let ix=   await  program.methods.claimLocked().accounts({
      ...
      await provider.connection.confirmTransaction(x)
      console.log("Claim created successfully:", x);



      let after_claim_userTokenAccount = await getOrCreateAssociatedTokenAccount(
        provider.connection, 
        mintOwner, 
        mint, 
        user.publicKey,
      ...
      console.log("user token account balance:", after_claim_userTokenAccount.amount.toString())
